...
class ImbalancedDatasetReg:
    ...
    def implementML(self, x_train, y_train, x_test, y_test, over=None, k=None):
        """
        ...
        Args:
            ...
            reg (class, optional): The regression class to be used. Defaults to None.
            over (str, optional): The name of the oversampling technique. Defaults to None.
            k (int, optional): The value of k for oversampling. Defaults to None.

        Returns:
            tuple: A tuple containing the cross-validation error and training error.

        Explanation:
            This method implements machine learning regression using the specified regressor. It fits the regressor on the training data, predicts the testing data, and calculates the mean absolute errors between the predicted and actual values. The cross-validation error and training error are returned as a tuple. If oversampling is specified, the predicted and actual values are saved to a CSV file with the name of the oversampling technique and the value of k.
    ...
    def no_resampling(self):
        ...
        for train_index, test_index in kfold.split(self.X):
            ...
            cv_err, train_err = self.implementML(x_train, y_train, x_test, y_test)
